        ...
        dir('contact-app-ui') {
          withCredentials([usernamePassword(credentialsId: 'docker-hub-creds', usernameVariable: 'DOCKER_USER', passwordVariable: 'DOCKER_PASS')]) {
            sh '''
              ...
              docker build -t $DOCKER_USER/contactsapp:ui-${IMAGE_TAG} .
              docker push $DOCKER_USER/contactsapp:ui-${IMAGE_TAG}

              docker tag $DOCKER_USER/contactsapp:ui-${IMAGE_TAG} $DOCKER_USER/contactsapp:ui-latest
              docker push $DOCKER_USER/contactsapp:ui-latest
            '''
          }
        }
      }
        ...
        dir('contact-app-api') {
          withCredentials([usernamePassword(credentialsId: 'docker-hub-creds', usernameVariable: 'DOCKER_USER', passwordVariable: 'DOCKER_PASS')]) {
            sh '''
              ...
              docker build -t $DOCKER_USER/contactsapp:api-${IMAGE_TAG} .
              docker push $DOCKER_USER/contactsapp:api-${IMAGE_TAG}

              docker tag $DOCKER_USER/contactsapp:api-${IMAGE_TAG} $DOCKER_USER/contactsapp:api-latest
              docker push $DOCKER_USER/contactsapp:api-latest
            '''
          }
        }
      }
